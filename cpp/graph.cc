...
void Graph::AddEdge(const Node& node_a, const Node& node_b, double weight) {
  AddDirectedEdge(node_a, node_b, weight);
  AddDirectedEdge(node_b, node_a, weight);
  num_edges_ += 1;
}

void Graph::AddDirectedEdge(const Node& start_node, const Node& end_node, double weight) {
...
bool Graph::HasEdge(const Node& node_a, const Node& node_b) const {
    ...
    if (line.size() > 0 && line[0] != '#') {
      ...
      while (std::getline(line_stream, field, ' ')) {
        if (first_field) {
          ...
          const Graph::Node& end_node = field;
          // Unweighted edges.
